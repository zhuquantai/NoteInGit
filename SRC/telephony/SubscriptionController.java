...
public class SubscriptionController extends ISub.Stub {
    ...
    // MTK-END
    private boolean isSpnShowFormatChange = false; //20151028 add by hlinjie for task779537 merge from defect288081
    ...
    private SubscriptionController(Context c) {
        ...
        if (operator.equals("OP01")) {
            ...
            /*-begin-shaodongbo-add-for-Task 716751 */
            boolean supportMultiColorSim = mContext.getResources().getBoolean(com.android.internal.R.bool.def_SIM_indicator_multi_color);
            if((!supportMultiColorSim)&&(colorArr != null)){
            	int defaultColorIdx = colorArr[0];
            	for(int i = 0;i < colorArr.length;i++){
            		colorArr[i] = defaultColorIdx;
            	}
            }
            /*-end-shaodongbo-add-for-Task 716751 */
        }
        // MTK-END
        ...
        isSpnShowFormatChange = SystemProperties.getBoolean("ro.idea.spn.show.format.enable", false); //20151028 add by hlinjie for task779537 merge from defect288081
    }
    ...
    public List<SubscriptionInfo> getActiveSubscriptionInfoList(String callingPackage) {
        ...
        try {
            ...
            List<SubscriptionInfo> subList = mActiveList;
            // MTK-END
            if (subList != null) {
        ...
        try {
           ...
           // MTK-END
            ...
            try {
                ...
                if (cursor != null) {
                    // MTK-START
                    // Prepare cache variable to enhance further query performance
                    // FIXME: need to revise if more than one application existed within
                    // a same slot id.
            ...
            if (setDisplayName) {
                ...
                if (!TextUtils.isEmpty(simMvnoName)
                        ...
                        && (!mContext.getResources().getBoolean(com.android.internal.R.bool.def_frameworks_use_spn_as_sim_name_enable)
                            || TextUtils.isEmpty(simCarrierName))) {
                ...
                // MTK-END
                    if (!TextUtils.isEmpty(simCarrierName)) {
                        ...
                        nameToSet = "SIM " + Integer.toString(slotId + 1);//jianping.zhang-20160106-modify from "CARD" to "SIM"  for Defect1275689,this modify will apply to all application
                    }
                // MTK-START
                }
                // MTK-END
            ...
            if (showPlmn) {
                ...
                if (showSpn) {
                    ...
                    if(!Objects.equals(spn, plmn)) {
                        ...
                        //begin 20151028 add by hlinjie for task779537 merge from defect288081
                        if (isSpnShowFormatChange) {
                            separator = " | ";
                        }
                        //end 20151028 add by hlinjie for task779537 merge from defect288081
    ...
    public int getSlotId(int subId) {
        ...
        for (Entry<Integer, Integer> entry: sSlotIdxToSubId.entrySet()) {
            ...
            if (subId == sub)
            {
                ...
                if (VDBG) logv("[getSlotId]- return =" + sim + ", subId = " + subId);
                return sim;
                // MTK-END
            }
        }
    ...
    public int getPhoneId(int subId) {
        ...
        if (MTK_LTEDC_SUPPORT) {
            if (subId == SubscriptionManager.LTE_DC_SUB_ID_1
                    || subId == SubscriptionManager.LTE_DC_SUB_ID_1 - 2) {
                logd("[getPhoneId]- subId is LTE_DC_SUB_ID_1.");
                return PhoneConstants.SIM_ID_1;
            } else if (subId == SubscriptionManager.LTE_DC_SUB_ID_2
                    || subId == SubscriptionManager.LTE_DC_SUB_ID_2 - 2) {
    ...
    public void setDefaultDataSubId(int subId) {
        ...
        if (SubscriptionManager.isValidSubscriptionId(subId)
                // MTK-START
                // VSIM feature will try to set a default data sub to non SIM card inserted slot.
            ...
            if (atLeastOneMatch) {
           //BEGIN-20151026-jianping.zhang-add-for-task730609
	     if("false".equalsIgnoreCase(SystemProperties.get("separate_data_and_3Gservice", "false"))
	     &&"false".equalsIgnoreCase(SystemProperties.get("def_simlock_cellcom_guine", "false")) //20151228-jianping.zhang-add-for-guinea-simlock	
	     &&"false".equalsIgnoreCase(SystemProperties.get("def_simlock_kyivstar_ukraine", "false"))) //20150901-jianping.zhang-add-for-task384601
           {
                proxyController.setRadioCapability(rafs); 
	  }
	  //END-20151026-jianping.zhang-add-for-task730609
            } else {
    ...
    public SubscriptionInfo getSubscriptionInfo(int subId) {
        if (!canReadPhoneState(mContext.getOpPackageName(), "getSubscriptionInfo")) {
            return null;
        }
    ...
    /**
     * NotifySubscriptionInfoChanged will detectType.
     * @param detectType the detect tpe
     */
    public void notifySubscriptionInfoChanged(Intent intent) {
        ITelephonyRegistry tr = ITelephonyRegistry.Stub.asInterface(ServiceManager.getService(
                "telephony.registry"));
        try {
            if (DBG) logd("notifySubscriptionInfoChanged with indicated intent:");
            tr.notifySubscriptionInfoChanged();
        } catch (RemoteException ex) {
            // Should never happen because its always available.
        }
    ...
    public void removeStickyIntent() {
        if (sStickyIntent != null) {
            if (DBG) logd("removeStickyIntent");
