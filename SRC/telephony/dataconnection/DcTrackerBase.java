...
public abstract class DcTrackerBase extends Handler {
   ...
   /* ALPS01555724: The implementation of PriorityQueue is incorrect, use arraylist to sort priority.
    ...
    private final static String ACTION_DISK_SPACE_LOWER_OFF = "storageforcemode.enable.dataconnection";
    private final static String ACTION_DISK_SPACE_LOWER_ON = "storageforcemode.disable.dataconnection";
    ...
    protected BroadcastReceiver mIntentReceiver = new BroadcastReceiver ()
    {
        ...
        public void onReceive(Context context, Intent intent)
        {
            ...
            if (action.equals(Intent.ACTION_SCREEN_ON)) {
                ...
                onActionIntentSimStateChanged(intent);
			/*Begin heyan add for Task 742719 low memory notification 20151210 */
			} else if (action.equals(ACTION_DISK_SPACE_LOWER_ON) || action.equals(ACTION_DISK_SPACE_LOWER_OFF)) {
                boolean isDiskLower = action.equals(ACTION_DISK_SPACE_LOWER_ON);
                Message msg = obtainMessage(DctConstants.EVENT_DISKSPACE_LOWER);
                msg.arg1 = isDiskLower ? 1 : 0;
                sendMessage(msg);
            }
			/*End heyan add for Task 742719 low memory notification 20151210 */
        }
    ...
    private class DataRoamingSettingObserver extends ContentObserver {
		...
		/*-begin-20150608-zubin.chen-add-for-df335160-*/
                int slotID = SubscriptionManager.getSlotId(phoneSubId);
	       if (slotID == 1) {
                    contentUri = Settings.Global.DATA_ROAMING_2;
                } else if(slotID == 0) {
                	contentUri = Settings.Global.DATA_ROAMING;
                }else{		
        ...
        public void onChange(boolean selfChange) {
            ...
            if (mPhone.getServiceState().getDataRoaming()) {
                ...
                mWorkerHandler.sendEmptyMessage(DctConstants.EVENT_DATA_ROAMING_CHANGED);
            }
        }
    }
    ...
    // M: SIM records loaded event needs to wait for subId ready,
    // so trigger onRecordsLoaded should be notified by SubscriptionInfoUpdater.
    protected void onActionIntentSimStateChanged(Intent intent) {
        String state = intent.getStringExtra(IccCardConstants.INTENT_KEY_ICC_STATE);
        int phoneId = intent.getIntExtra(PhoneConstants.PHONE_KEY, PhoneConstants.SIM_ID_1);
        // As the phoneId is slotId in actual from intent.
        ...
        if (IccCardConstants.INTENT_VALUE_ICC_LOADED.equals(state)
                && phoneId == curPhoneId) {
            if (DBG) {
                log("onActionIntentSimStateChanged: phoneId=" + phoneId +
                        " state LOADED and record load completed");
            }
    ...
    protected DcTrackerBase(PhoneBase phone) {
        ...
        /*Begin heyan add for Task 742719 low memory notification 20151210 */
		filter.addAction(ACTION_DISK_SPACE_LOWER_ON);
        filter.addAction(ACTION_DISK_SPACE_LOWER_OFF);
        /*End heyan add for Task 742719 low memory notification 20151210 */
    ...
    protected ApnSetting fetchDunApn() {
        ...
        /*-BEGIN-20151027-qingtao.wen-modify-for-task745075-*/
        //String[] apnArrayData = c.getResources().getStringArray(R.array.config_tether_apndata);
        String[] apnArrayData = new String[3];
        apnArrayData[0] = c.getResources().getString(com.android.internal.R.string.def_tether_dun_apn_cust1);
        apnArrayData[1] = c.getResources().getString(com.android.internal.R.string.def_tether_dun_apn_cust2);
        apnArrayData[2] = c.getResources().getString(com.android.internal.R.string.def_tether_dun_apn_cust3);
        if (VDBG) log("fetchDunApn: customzied apn= " + apnArrayData[0] + " " + apnArrayData[1] + " " + apnArrayData[2]);
        /*-END-20151027-qingtao.wen-modify-for-task745075-*/
        for (String apn : apnArrayData) {
            ApnSetting dunSetting = ApnSetting.fromString(apn);
            if (dunSetting != null) {
                if (!ServiceState.bitmaskHasTech(dunSetting.bearerBitmask, bearer)) continue;
                //min.zhang.hz add for 1839105 20160317 begin
                String operator = (r != null) ? r.getOperatorNumeric() : "";
                //if (VDBG) log("fetchDunApn: operator= " + operator + " dunSetting.numeric= " + dunSetting.numeric);
                if (!dunSetting.numeric.equals(operator))continue;
                //min.zhang.hz add for 1839105 20160317 end
                if (dunSetting.hasMvnoParams()) {
                    if (r != null && ApnSetting.mvnoMatches(r, dunSetting.mvnoType,
                            dunSetting.mvnoMatchData)) {
                        if (VDBG) {
...
/*-begin-20150608-zubin.chen-add-for-df335160-*/
	        int slotID = SubscriptionManager.getSlotId(phoneSubId);
	        if (slotID == 1) {
	        	Settings.Global.putInt(mResolver, Settings.Global.DATA_ROAMING_2, roaming);
	        } else {
	        	Settings.Global.putInt(mResolver, Settings.Global.DATA_ROAMING, roaming);
	        } 
/*-END-20150608-zubin.chen-add-for-df335160-*/		
            }
    ...
    public boolean getDataOnRoamingEnabled() {
        ...
        try {
            // For single SIM phones, this is a per phone property.
        	/*-begin-20151020-zubin.chen-add-for-df730248-*/
            if (TelephonyManager.getDefault().getSimCount() == 1) {
                isDataRoamingEnabled = Settings.Global.getInt(mResolver,
                        Settings.Global.DATA_ROAMING, isDataRoamingEnabled ? 1 : 0) == SubscriptionManager.DATA_ROAMING_ENABLE;
            } else {
                isDataRoamingEnabled = TelephonyManager.getIntWithSubId(mResolver,
                        Settings.Global.DATA_ROAMING, phoneSubId) == SubscriptionManager.DATA_ROAMING_ENABLE;
		        int slotID = SubscriptionManager.getSlotId(phoneSubId);
		        if (slotID == 1) {
	                isDataRoamingEnabled = Settings.Global.getInt(mResolver,
	                        Settings.Global.DATA_ROAMING_2, isDataRoamingEnabled ? 1 : 0) == SubscriptionManager.DATA_ROAMING_ENABLE;
		        } else {
	                isDataRoamingEnabled = Settings.Global.getInt(mResolver,
	                        Settings.Global.DATA_ROAMING, isDataRoamingEnabled ? 1 : 0) == SubscriptionManager.DATA_ROAMING_ENABLE;
		        }                 
            }
            /*-end-20151020-zubin.chen-add-for-df730248-*/
    ...
    //Begin heyan add for Task 742719 low memory notification 20151210
    protected void onDiskSpaceLower(boolean isDiskSpaceLow) {};
    //End heyan add for Task 742719 low memory notification 20151210
    ...
    public void handleMessage(Message msg) {
        switch (msg.what) {
            ...
            /* Begin heyan add for Task 742719 low memory notification 20151210 */
            case DctConstants.EVENT_DISKSPACE_LOWER:
                onDiskSpaceLower(msg.arg1 == 1);
                break;
            /* End heyan add for Task 742719 low memory notification 20151210 */
        ...
        synchronized (mDataEnabledLock) {
            if (mUserDataEnabled != enabled) {
                ...
                if ((getDataOnRoamingEnabled() == false && mPhone.getServiceState().getRoaming() == true) ||
                        (isDomesticRoamingEnabled() == false && isDomesticRoaming() == true)) {
    ...
    private boolean isDomesticRoamingEnabled() {
        int phoneSubId = mPhone.getSubId();
        if (!SubscriptionManager.isValidSubscriptionId(phoneSubId)) {
        	log("[getSubId]- fail, phoneSubId = " + phoneSubId);
        	return false;
        }
        try {
            ...
            if (TelephonyManager.getDefault().getSimCount() == 1) {
		        ...
		        if (slotID == 1) {
	                ...
	                return TelephonyManager.getIntWithSubId(resolver,
	                        ...
	                        == SubscriptionManager.DATA_NATIONAL_ROAMING_ENABLE;
		        }
            }
        } catch (SettingNotFoundException snfe) {
            return false;
    ...
    private void setInitialAttachApnForSvlte() {
        ...
        log("[IRAT_DcTracker] setInitialAttachApnForSvlte: apn = "
                + mSvlteIaApnSetting);
         IaExtendParam param = new IaExtendParam(mSvlteOperatorNumeric);
