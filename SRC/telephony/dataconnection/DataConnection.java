...
public final class DataConnection extends StateMachine {
    ...
    static final int EVENT_ADDRESS_REMOVED = BASE + 21;

    static final int EVENT_DATA_CONNECTION_VOICE_CALL_STARTED = BASE + 22;
    static final int EVENT_DATA_CONNECTION_VOICE_CALL_ENDED = BASE + 23;
    ...
    static DataConnection makeDataConnection(PhoneBase phone, int id,
        ...
        dc.regiseterNetworkAlertObserver();
    ...
    private class DcDefaultState extends State {
        ...
        public void enter() {
            ...
            if (CdmaFeatureOptionUtils.isCdmaLteDcSupport()) {
                SvlteUtils.getSvltePhoneProxy(mPhone.getPhoneId()).getSvlteSstProxy()
                        .registerForDataRegStateOrRatChanged(
                                getHandler(),
                                DataConnection.EVENT_DATA_CONNECTION_DRS_OR_RAT_CHANGED,
                                null);
            } else {
        ...
        public void exit() {
            ...
            if (CdmaFeatureOptionUtils.isCdmaLteDcSupport()) {
                SvlteUtils.getSvltePhoneProxy(mPhone.getPhoneId()).getSvlteSstProxy()
                        .unregisterForDataRegStateOrRatChanged(getHandler());
            } else {
    ...
    private boolean updateNetworkInfoSuspendState() {
        ...
        /// M: Fix the problem when MD-iRAT, the rat change event is before mPhone
        // update event. The sst to be exepected as SvlteServiceStateTracker
        // TODO: In AOSP, the code to be removed as only
        // cdmaLtePhoneServiceStateTracker existed.
        if (CdmaFeatureOptionUtils.isCdmaLteDcSupport()
                && SvlteUtils.isActiveSvlteMode(mPhone)) {
            PhoneBase pb = (PhoneBase) SvlteUtils.
                                       getSvltePhoneProxy(mPhone.getPhoneId()).
                                       getCsPhone();
            sst = pb.getServiceStateTracker();
        }
        log("updateNetworkInfoSuspendState sst = " + sst);
            ...
            switch (msg.what) {
                case EVENT_DATA_CONNECTION_DRS_OR_RAT_CHANGED:
                    ...
                    if ((rat == mRilRat) && (drs == mDataRegState)) {
                        ...
                        if (CdmaFeatureOptionUtils.isCdmaLteDcSupport()) {
                            //can get networkType from svltePhoneProxy to get right network type
                            ServiceState svlteSs =
                                    SvlteUtils.getSvltePhoneProxy(mPhone.getPhoneId())
                                            .getSvlteServiceState();
                            if (svlteSs != null) {
                                ss = svlteSs;
                            }
        ...
        public boolean processMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case EVENT_LOST_CONNECTION: {
                    ...
                    if (mRetryManager.isRetryNeeded() && msg.arg2 == 1) {
                        // msg.arg2 --> DcController.isLoaded()
