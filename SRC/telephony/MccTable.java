...
public final class MccTable {
    ...
    //add by shilei.zhang for fixed Android M SIM mcc set system language task 736294 begin
    static int def=SystemProperties.getInt("def.set.language.from.sim",-1);
    //ISDM def.set.systemlanguage.by.ef Whether to set mobile's language according to the ef(li and pl) of SIM card. 0x00:disable, don't set system language by ef, 0x01:enable, set system language by ef. Default value:0x00
    static int mSystemLanguageSetByEF=SystemProperties.getInt("def.set.systemlanguage.by.ef",-1);
    //ISDM def.set.systemlanguage.by.plmn Whether to set mobile's language according to the PLMN of SIM card. 0x00:NO set language by perso, 0x01:YES set language by PLMN. Default value:0x01.
    static int mSystemLanguageSetByPLMN = SystemProperties.getInt("def.set.systemlanguage.by.plmn",-1);
    private static final String MCC_BOOT_SYS_PROPERTY = "persist.sys.mcc.first_time_boot";
    //add by shilei.zhang for fixed Android M SIM mcc set system language task 736294 begin
...
//            }
	     ...
	     //modify by shilei.zhang for fixed Android M SIM mcc set system language task 736294 begin
	     Locale locale = null;
	     if (mcc != 0) {
            ...
            if (fromServiceState) {
                ...
                try {
                    ...
                    if (locale != null) {
				Slog.d(LOG_TAG, "updateMccMncConfiguration: DDD- local language=" + locale.toLanguageTag());
				//persist.sys.simsetlanguage.M for sim set language customization
			       SystemProperties.set("persist.sys.simsetlanguage.M", locale.toLanguageTag());//modify by shilei.zhang for fromServiceState is true fixed D1146031
				config.setLocale(locale);
				updateConfig = true;
		      }

		      //modify by shilei.zhang for fixed Android M SIM mcc set system language task 736294 end
                    if (updateConfig) {
    ...
    //add by shilei.zhang for user set language records for task 736294 begin
    private static boolean userHasPersistedLocale() {
	 ...
	 Slog.d(LOG_TAG, "userHasPersistedLocale persistSyslocale:"+ persistSyslocale);
        //return !(persistSysLanguage.isEmpty() && persistSysCountry.isEmpty() && persistSyslocale.isEmpty());
        return !(persistSyslocale.isEmpty());
    }
    //add by shilei.zhang for user set language records for task 736294 end
    /**
    ...
    private static Locale getLocaleForLanguageCountry(Context context, String language,
        ...
        // add code by shilei.zhang to return null in sometimes for task 736294 begin
        // If this is a regular user and they already have a persisted locale, we're done.
        //add (0 == mSystemLanguageSetByPLMN) for fixed D416045 if def.set.systemlanguage.by.plmn is 0, do not read locale
        if (userHasPersistedLocale() || (0 == mSystemLanguageSetByPLMN)) {
			//add by shilei.zhang for get  info begin
			Slog.d(LOG_TAG, "getLocaleForLanguageCountry: not permitted to update locale");
			if(userHasPersistedLocale()){
				Slog.d(LOG_TAG, "getLocaleForLanguageCountry: userHasPersistedLocale");
				return null;
			}else if(0 == mSystemLanguageSetByPLMN){
				Slog.d(LOG_TAG, "getLocaleForLanguageCountry: mSystemLanguageSetByPLMN");
				return null;
			}else{
				return null;
			}
        }
	 //when set def=1 then mobile will change locale language according to MCC of SIM card each time
	 if(def !=1 && (!SystemProperties.get("persist.sys.simsetlanguage.M").isEmpty())){
		Slog.d(LOG_TAG, "DDD-getLocaleForLanguageCountry def !=1 && persist.sys.simsetlanguage.M ok");
              return null;
        }
	 if(def ==1){
		Slog.d(LOG_TAG, "DDD-getLocaleForLanguageCountry def ==1");
	 ...
	 if(SystemProperties.get("persist.sys.simsetlanguage.M").isEmpty()){
		Slog.d(LOG_TAG, "DDD-getLocaleForLanguageCountry persist.sys.simsetlanguage.M nok");

        }
	 if(!SystemProperties.get("persist.sys.simsetlanguage.M").isEmpty()){
		Slog.d(LOG_TAG, "DDD-getLocaleForLanguageCountry persist.sys.simsetlanguage.M ok");

        }
	 // add code by shilei.zhang to return null in sometimes for task 736294 end
    ...
    /**
    *add by shilei.zhang for set system languaeg by sim before PIN code and fixed D1658045
    ...
    */
    public static void setSystemLocale(Context context, String language, String country) {
        if (1 != mSystemLanguageSetByEF) {
                Slog.d(LOG_TAG, "setSystemLocale: mSystemLanguageSetByEF is not 1");
                return;
        }
        //get locale
    ...
    public static Locale getLocaleFromMcc(Context context, int mcc, String simLanguage) {
	 ...
	 //modify by shilei.zhang for fixed task 736294 begin
	 String tempString = SystemProperties.get(MCC_BOOT_SYS_PROPERTY, "");
        boolean isFirstBoot = (tempString != null && "".equals(tempString)) ? true : false;
	 if(isFirstBoot)
        {
              Slog.d(LOG_TAG, "DDD-getLocaleFromMcc  first time boot");
              SystemProperties.set("persist.sys.simsetlanguage.M", "");//TCL_shilei:persist.sys.simsetlanguage.M set to ""
	       SystemProperties.set(MCC_BOOT_SYS_PROPERTY, "false");
        }
	 ...
	 /*if(locale == null){
	      Slog.d(LOG_TAG, "DDD-getLocaleFromMcc  locale == null");
	 }else{
	      //final String languageTag = Locale.getDefault().toLanguageTag();
	      //SystemProperties.set("persist.sys.simsetlanguage.M", languageTag);//move this code to updateMccMncConfiguration TCL_shilei:persist.sys.simsetlanguage.M for sim set language customization
	      Slog.d(LOG_TAG, "DDD-getLocaleFromMcc  languageTag = "+languageTag);
	}*/
       //modify by shilei.zhang for fixed task 736294 end
        //Slog.d(LOG_TAG, "getLocaleFromMcc(" + language + ", " + country + ", " + mcc);
    ...
    static {
		...
		sTable.add(new MccEntry(630,"cd",2));   //Democratic Republic of the Congo,modify by weiyong for defect1616559
