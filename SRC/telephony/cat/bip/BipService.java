...
public class BipService {
    ...
    public BipService(Context context, Handler handler, int sim_id) {
        ...
        if (context == null) {
            CatLog.d("[BIP]", "Fail to construct BipService");
        }
   ...
   public void dispose() {
    ...
    void setBipEventList(int eventId) {
        switch (eventId) {
            case DATA_AVAILABLE_EVENT:
                mIsListenDataAvailable = true;
                break;
    ...
    boolean hasPsEvent(int eventId) {
        switch (eventId) {
            case DATA_AVAILABLE_EVENT:
                CatLog.d(this,"mIsListenDataAvailable:" + mIsListenDataAvailable);
                return mIsListenDataAvailable;
            case CHANNEL_STATUS_EVENT:
                CatLog.d(this,"mIsListenChannelStatus:" + mIsListenChannelStatus);
                return mIsListenChannelStatus;
            default:
                break;
        }
        CatLog.d(this,"Unknow eventId:" + eventId);
        return false;
    }
    /**
                    ...
                    try {
                        ...
                        if (BipUtils.TRANSPORT_PROTOCOL_UDP_REMOTE == channel.mProtocolType ||
                            ...
                            if (SystemProperties.get("ro.mtk_gemini_support").equals("1") == true) {
                                ...
                                //NETWORK_TYPE, Phone.FEATURE_ENABLE_SUPL, mSlotId);
                                releaseRequest(mNetworkCallback);
                                resetLocked();
                            } else {
                                //mConnMgr.stopUsingNetworkFeature(
                                //NETWORK_TYPE, Phone.FEATURE_ENABLE_SUPL);
                                releaseRequest(mNetworkCallback);
                                resetLocked();
                            }
                            channel.closeChannel();
...
/*
            if (SystemProperties.get("ro.mtk_gemini_support").equals("1") == true) {
                if (getDataConnectionFromSetting() == mSlotId) {
                    ...
                    result = mConnMgr.startUsingNetworkFeatureGemini(NETWORK_TYPE,
                    Phone.FEATURE_ENABLE_SUPL, mSlotId);
                }
            }else{
                // result = mConnMgr.startUsingNetworkFeatureGemini(NETWORK_TYPE,
                // Phone.FEATURE_ENABLE_SUPL,mPhone.getMySimId());
            ...
            if(result == PhoneConstants.APN_ALREADY_ACTIVE) {
                ...
                Message timerMsg = mBipSrvHandler.obtainMessage(MSG_ID_CONN_RETRY_TIMEOUT);
                timerMsg.obj = cmdMsg;
                mBipSrvHandler.sendMessageDelayed(timerMsg, CONN_RETRY_TIMEOUT);
            }
*/
        }
                ...
                if (BipUtils.CHANNEL_STATUS_UNKNOWN ==
                    ...
                    if(null == lChannel) {
                        ...
                        if (BipUtils.TRANSPORT_PROTOCOL_SERVER == lChannel.mProtocolType) {
                            ...
                            if (SystemProperties.get("ro.mtk_gemini_support").equals("1") == true) {
                                CatLog.d("[BIP]",
                                         "stopUsingNetworkFeature getDataConnectionFromSetting  =="
                                         + mSlotId);
...
/*
        ...
        if(BipUtils.LINK_ESTABLISHMENT_MODE_ONDEMMAND == channel.mLinkMode) {
            ...
            if (SystemProperties.get("ro.mtk_gemini_support").equals("1") == true) {
                if (getDataConnectionFromSetting() == mSlotId) {
                    CatLog.d("[BIP]", "Start to establish data connection" + mSlotId);
                    result = mConnMgr.startUsingNetworkFeatureGemini(
                            NETWORK_TYPE, Phone.FEATURE_ENABLE_SUPL, mSlotId);
                }
            }else{
                // result = mConnMgr.startUsingNetworkFeatureGemini(
                //         NETWORK_TYPE, Phone.FEATURE_ENABLE_SUPL,mPhone.getMySimId());
                result = mConnMgr.startUsingNetworkFeature(NETWORK_TYPE, Phone.FEATURE_ENABLE_SUPL);
            }
            if(result == PhoneConstants.APN_ALREADY_ACTIVE) {
                CatLog.d("[BIP]", "BM-openChannel: APN already active");
                ...
                Message timerMsg = mBipSrvHandler.obtainMessage(MSG_ID_CONN_MGR_TIMEOUT);
                timerMsg.obj = cmdMsg;
                mBipSrvHandler.sendMessageDelayed(timerMsg, CONN_MGR_TIMEOUT);
            } else {
                CatLog.d("[BIP]", "BM-openChannel: startUsingNetworkFeature FAIL");
                ret = ErrorValue.NETWORK_CURRENTLY_UNABLE_TO_PROCESS_COMMAND;
                cmdMsg.mChannelStatusData.isActivated = false;
