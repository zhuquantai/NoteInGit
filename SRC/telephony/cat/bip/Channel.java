...
abstract class Channel {
    ...
    Channel(int cid, int linkMode, int protocolType, InetAddress address, int port, int bufferSize,
            CatService handler, BipService bipManager) {
        this.mChannelId = cid;
    ...
    public void dataAvailable(int bufferSize) {
        ...
        if (!mBipService.hasPsEvent(DATA_AVAILABLE_EVENT)) {
            CatLog.d(this, "No need to send data available.");
            return;
        }
        //ToDo: get SetEventListParams and CatCmdMessage by BipService.
        CatResponseMessage resMsg = new CatResponseMessage(DATA_AVAILABLE_EVENT);
        ...
        resMsg.setEventDownload(DATA_AVAILABLE_EVENT, additionalInfo);
    ...
    public void changeChannelStatus(byte status) {
        ...
        if (!mBipService.hasPsEvent(CHANNEL_STATUS_EVENT)) {
            CatLog.d(this, "No need to send channel status.");
            return;
        }
        //ToDo: get SetEventListParams and CatCmdMessage by BipService.
        CatResponseMessage resMsg = new CatResponseMessage(CHANNEL_STATUS_EVENT);
    ...
    protected int checkBufferSize() {
        ...
        CatLog.d("[BIP]", "mBufferSize:" + mBufferSize + " minBufferSize:" + minBufferSize +
                ...
                while (!mStop) {
                    ...
                    if (recvLen >= 0) {
                        ...
                        synchronized (mLock)
                        {
                            ...
                            {
                                //shift the data to the head of array.
                                ...
                                mRxBufferOffset = 0;
                                CatLog.d("[BIP]", "RecTr:rSize: " + rSize + ", mRxBufferCount: " +
                                         mRxBufferCount);
                            }
                            do {
                                /* For IOT test, the UE can only recevie next packet
                                  if the SIM has consumed current packet totally. */
                                if (1 == mIOTTest) {
                                     if (0 < mRxBufferCount) {
                                        CatLog.d("[BIP]", "[UDP]0 < mRxBufferCount, wait.");
                                        mLock.wait();
                                        if (!isReceiveDataTRSent) {
                                            CatLog.d("[BIP]", "[UDP]Receive TR not sent, wait.");
                                            mLock.wait();
                                        }
                                     } else {
                                        break;
                                    ...
                                    if (mRxBufferCount >= mBufferSize) {
                                        try {
                                            ...
                                            CatLog.e("[BIP]",
                                                  "[UDP]RecTr:IE : mRxBufferCount >= mBufferSize");
                                                //break;
                                        }
                                        if (0 < mRxBufferCacheCount) {
                                            ...
                                            if (mRxBufferCacheCount <= (mBufferSize -
                                                                        mRxBufferCount))
                    ...
                    if (recvLen >= 0) {
                        ...
                        synchronized (mLock)
                        {
                            ...
                            else
                            {
                                ...
                                mRxBufferCount += rSize; //recvLen + mRxBufferCount;
                                mRxBufferOffset = 0;
                                CatLog.d("[BIP]", "[TCP]RecTr:rSize: " + rSize +
                                         ", mRxBufferCount: " + mRxBufferCount +
                            ...
                            do {
                                if (mRxBufferCount >= mBufferSize) {
                                    try {
                                        ...
                                        CatLog.e("[BIP]",
                                                 "[TCP]RecTr:IE :mRxBufferCount >= mBufferSize");
                                            //break;
                                    }
                                    if (0 < mRxBufferCacheCount) {
                                        if (0 < mRxBufferCount)
                                            System.arraycopy(mRxBuffer, mRxBufferOffset, mRxBuffer,
                                                             0, mRxBufferCount);
                                        ...
                                        System.arraycopy(localBuffer, localBufferOffset, mRxBuffer,
                                                         mRxBufferCount, rSize);
                                        mRxBufferCount += rSize;
                                        mRxBufferCacheCount -= rSize;
                                        localBufferOffset += rSize;
...
/*
                        clearChannelBuffer(false);
                        //change state to avoid SAT attempts to
                        //send data to server and return BIP error.
                        mChannelStatus = BipUtils.CHANNEL_STATUS_LINK_DROPPED;
                        //change state for get channel status command.
                        mChannelStatusData.mChannelStatus = ChannelStatus.CHANNEL_STATUS_NO_LINK;
                        //We wait SAT to launch close channel proactive command to close BIP channel
                        closeChannel();
            ...
            for (;; ) {
                if (mChannelStatus == BipUtils.CHANNEL_STATUS_OPEN) {
                    ...
                    try {
                        ...
                        CatLog.e("[BIP]", "[UICC]ServerTr:Fail to accept server socket retry:" +
                                 mReTryCount);
                    ...
                    while (!mStop) {
                        ...
                        if (recvLen >= 0) {
                            ...
                            synchronized (mLock) {
                                ...
                                CatLog.d("[BIP]", "[UICC]ServerTr:mRxBufferCount: " +
                                        mRxBufferCount);
                                if (mRxBufferCount == 0) {
                                    System.arraycopy(localBuffer, 0, mRxBuffer, 0, recvLen);
                                    mRxBufferCount = recvLen;
                                    ...
                                    mRxBufferCount += rSize; //recvLen + mRxBufferCount;
                                    mRxBufferOffset = 0;
                                    CatLog.d("[BIP]", "[UICC]ServerTr:rSize: " + rSize +
                                            ", mRxBufferCacheCount: " + mRxBufferCacheCount);
                                }
                                do {
                                    if (mRxBufferCount >= mBufferSize) {
...
class TcpServerChannel extends Channel {
    ...
    public int openChannel(BipCmdMessage cmdMsg, Network network) {
        ...
        try {
            CatLog.d("[BIP]", "[UICC]New server socket.mChannelStatus:" + mChannelStatus +
                     ",port:" + mPort);
    ...
    public ReceiveDataResult receiveData(int requestCount) {
        ...
        CatLog.d("[BIP]", "[UICC]receiveData " + mRxBufferCount +
    ...
    public int sendData(byte[] data, int mode) {
        ...
        CatLog.d("[BIP]", "[UICC]sendData: size of buffer:" + mTxBuffer.length + " count:" +
                 mTxBufferCount);
        ...
        if (mode == BipUtils.SEND_DATA_MODE_IMMEDIATE &&
            ...
            try {
                CatLog.d("[BIP]", "S[UICC]END_DATA_MODE_IMMEDIATE:" + mTxBuffer.length +
    ...
    public int receiveData(int requestSize, ReceiveDataResult rdr) {
        ...
        CatLog.d("[BIP]", "[UICC]receiveData " + mRxBufferCount + "/" + requestSize +
                 "/" + mRxBufferOffset);
    ...
    public void setTcpStatus(byte status, boolean isPackED) {
        if (mChannelStatusData.mChannelStatus == status) {
            ...
            CatLog.d("[BIP]", "[UICC][TCPStatus]" + mChannelStatusData.mChannelStatus +
                     "->" + status);
...
class TcpChannel extends Channel {
    ...
    TcpChannel(int cid, int linkMode, int protocolType, InetAddress address, int port,
               int bufferSize, CatService handler, BipService bipManager) {
            ...
            t_openChannelThread = new Thread(new Runnable() {
                public synchronized void run() {
                    ...
                    try {
                        ...
                        if (mBipService.isTestSim()) {
                            //Bind UE port to 32000 to fit Anite 4G BIP test case.
                        ...
                        try {
                            ...
                            CatLog.d("[BIP]", "[TCP]Time out of connect " + e3 + ":" +
    ...
    public ReceiveDataResult receiveData(int requestCount) {
        ReceiveDataResult ret = new ReceiveDataResult();
        ret.buffer = new byte[requestCount];
        CatLog.d("[BIP]", "[TCP]receiveData " + mRxBufferCount + "/" + requestCount +
                 "/" + mRxBufferOffset);
    ...
    public int sendData(byte[] data, int mode) {
        ...
        try {
            ...
            CatLog.d("[BIP]", "[TCP]sendData: size of buffer:" + mTxBuffer.length +
                     " count:" + mTxBufferCount);
            ...
            if (mode == BipUtils.SEND_DATA_MODE_IMMEDIATE &&
                    mChannelStatus == BipUtils.CHANNEL_STATUS_OPEN) {
                try {
                    CatLog.d("[BIP]", "[TCP]SEND_DATA_MODE_IMMEDIATE:" + mTxBuffer.length +
                             " count:" + mTxBufferCount);
                    mOutput.write(tmpBuffer, 0, mTxBufferCount);
                    mOutput.flush();
                    mTxBufferCount = 0;
    ...
    public int receiveData(int requestSize, ReceiveDataResult rdr) {
        ...
        CatLog.d("[BIP]", "[TCP]receiveData " + mRxBufferCount + "/" + requestSize +
                 "/" + mRxBufferOffset);
...
class UdpChannel extends Channel {
    ...
    public ReceiveDataResult receiveData(int requestCount) {
        ...
        CatLog.d("[BIP]", "[UDP]receiveData " + mRxBufferCount + "/" + requestCount +
                 "/" + mRxBufferOffset);
    ...
    public int sendData(byte[] data, int mode) {
        ...
        CatLog.d("[BIP]", "[UDP]sendData: size of buffer:" + mTxBuffer.length +
                 " count:" + mTxBufferCount);
        try {
        if (0 == mTxBufferCount && BipUtils.SEND_DATA_MODE_IMMEDIATE == mode) {
            tmpBuffer = data;
        ...
        if (mode == BipUtils.SEND_DATA_MODE_IMMEDIATE) {
            CatLog.d("[BIP]", "[UDP]Send data(" + mAddress + ":" + mPort + "):" +
    ...
    public int receiveData(int requestSize, ReceiveDataResult rdr) {
        ...
        CatLog.d("[BIP]", "[UDP]receiveData " + mRxBufferCount + "/" + requestSize +
                 "/" + mRxBufferOffset);
