            ...
            if (AppInterface.UTK_SETUP_EVENT_LIST_ACTION.equals(intent.getAction())) {
                ...
                if (eventList != null && eventList.length > 0) {
                    for (int i = 0; i < eventList.length; i++) {
                        CatLog.d(this, "eventList[" + i + "] = " + eventList[i]);
                        mBipService.setBipEventList(eventList[i]);
                    }
                    mEventList = eventList;
                } else {
                    CatLog.d(this, "eventList is null !!!");
                }
            }
        }
    };
        ...
        switch (cmdParams.getCommandType()) {
            ...
            case PROVIDE_LOCAL_INFORMATION:
                ...
                if (cmdParams.mCmdDet.commandQualifier == CommandParamsFactory.DTTZ_SETTING) {
                    ...
                    /*begin-20151123-yangning.hong-merge-from-defect225819*/
                    //temp = cal.get(Calendar.ZONE_OFFSET) / (15 * 60 * 1000);
                    //hibyte = temp / 10;
                    //lobyte = (temp % 10) << 4;
                    temp = (cal.get(Calendar.ZONE_OFFSET) + cal.get(Calendar.DST_OFFSET))
                            / (15 * 60 * 1000);
                    if (temp < 0) {
                        temp = -temp;
                        hibyte = temp /10;
                    ...
                    /*end-20151123-yangning.hong-merge-from-defect225819*/
    ...
    private void eventDownload(int event, int sourceId, int destinationId,
        ...
        if (CdmaFeatureOptionUtils.isCdmaLteDcSupport()) {
            ...
            if (SvlteModeController.RADIO_TECH_MODE_SVLTE == radioTechMode) {
                if (DATA_AVAILABLE_EVENT != event &&
                        CHANNEL_STATUS_EVENT != event) {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (TelephonyIntents.ACTION_SIM_STATE_CHANGED.equals(intent.getAction())) {
                ...
                if (cardType.contains("RUIM") || cardType.contains("CSIM")) {
                    LteDcPhoneProxy lLteDcPhoneProxy =
                            (LteDcPhoneProxy) PhoneFactory.getPhone(mSlotId);
