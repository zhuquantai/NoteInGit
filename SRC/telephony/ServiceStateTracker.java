...
public abstract class ServiceStateTracker extends Handler {
    ...
    protected static final int EVENT_REG_SUSPENDED = 114;
    protected static final int EVENT_RESUME_CAMPING = 115;
    protected static final int EVENT_GET_SIGNAL_STRENGTH_ONLY = 116;
    ...
    */
    ...
    public boolean isPsRegStateRoamByUnsol() { return false; }
    ...
    protected int getPreferredNetworkModeSettings(int phoneId) {
        ...
        if (subId != null && SubscriptionManager.isValidSubscriptionId(subId[0])) {
            networkType = PhoneFactory.calculatePreferredNetworkType(
                mPhoneBase.getContext(), subId[0]);
        } else {
            if (CdmaFeatureOptionUtils.isCdmaLteDcSupport()) {
                int capabilityPhoneId = SystemProperties.getInt(
                          PhoneConstants.PROPERTY_CAPABILITY_SWITCH, 1);
                log("capabilityPhoneId=" + capabilityPhoneId + ", phoneId=" + phoneId);
                if (TelephonyManager.getDefault().getPhoneCount() == 1
                    || phoneId == (capabilityPhoneId - 1)) {
                    // Determine if the Icc card exists
                    boolean iccCardExist = false;
                    if (mUiccApplcation != null) {
                        iccCardExist = mUiccApplcation.getState() != AppState.APPSTATE_UNKNOWN;
                    ...
                    // Modem default is LTE only, so need to set RAT to
                    // allow emergency call when card absent
                    if (!iccCardExist) {
                        networkType = PhoneFactory.calculatePreferredNetworkType(
                            mPhoneBase.getContext());
    ...
    /**
     * get the card type for different slot id.
     * @param simId simId for different card
     * @return the card type for different slot id.
     */
    ...
    /**
     * Whether contains cdma apps or not.
     * @param cardType the cardType for different slot
     * @return true this card has cdma app
     */
    ...
    /**
     * Whether contains Usim app or not.
     * @param cardType the cardType of the SIM card
     * @return true if contains Usim app
     */
