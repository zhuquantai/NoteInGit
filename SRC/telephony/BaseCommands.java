...
/// M: For multiuser to register suspended event @{
...
public abstract class BaseCommands implements CommandsInterface {
    ...
    protected RegistrantList mSuspendedSetStatusRegistrants = new RegistrantList();
    ...
    protected boolean mIsIMEILock = false;
    // MTK-START, SMS part
    ...
    private static int STEP_TEST_WEIWEN = 0; // add by weiwen.huang.hz 20160114 alm1177030
    ...
    public void setOnNewGsmBroadcastSms(Handler h, int what, Object obj) {
        ...
        Rlog.d("RILJ", "weiwen "+ (STEP_TEST_WEIWEN++) +" setOnNewGsmBroadcastSms h: " + h);
        Rlog.d("RILJ", "weiwen "+ (STEP_TEST_WEIWEN++) +" setOnNewGsmBroadcastSms what: " + what);
        Rlog.d("RILJ", "weiwen "+ (STEP_TEST_WEIWEN++) +" setOnNewGsmBroadcastSms obj: " + obj);
        //add by weiwen.huang.hz 20151230 1177030 end
        mGsmBroadcastSmsRegistrant = new Registrant (h, what, obj);
        Rlog.d("RILJ", "weiwen "+ (STEP_TEST_WEIWEN++) +" setOnNewGsmBroadcastSms mGsmBroadcastSmsRegistrant:"+mGsmBroadcastSmsRegistrant);
    }

    @Override
    public void unSetOnNewGsmBroadcastSms(Handler h) {
        if (mGsmBroadcastSmsRegistrant != null && mGsmBroadcastSmsRegistrant.getHandler() == h) {
            Rlog.d("RILJ", "weiwen "+ (STEP_TEST_WEIWEN++) +" unSetOnNewGsmBroadcastSms clear!");//add by weiwen.huang.hz 20151230 1177030
            mGsmBroadcastSmsRegistrant.clear();
            mGsmBroadcastSmsRegistrant = null;
            Rlog.d("RILJ", "weiwen "+ (STEP_TEST_WEIWEN++) +" unSetOnNewGsmBroadcastSms mGsmBroadcastSmsRegistrant:"+mGsmBroadcastSmsRegistrant);
        }
    }
    ...
    */
    ...
    public void setOnRegistrationSuspended(Handler h, int what, Object obj) {
        synchronized (mWPMonitor) {
            ...
            // For multiuser to register suspended event - start
            ArrayList<Object> result = new ArrayList<Object>();
            result.add(new Boolean(true));
            result.add(h);
            if (mSuspendedSetStatusRegistrants != null) {
                mSuspendedSetStatusRegistrants.notifyRegistrants(
                        new AsyncResult(null, result, null));
            }
            // For multiuser to register suspended event - end
        }
    }

    public void unSetOnRegistrationSuspended(Handler h) {
        synchronized (mWPMonitor) {
            if (mRegistrationSuspendedRegistrant.getHandler() == h) {
    ...
    /// M: For multiuser to register suspended event @{
    public void registerForSuspendedSetStatus(Handler h, int what, Object obj) {
        Registrant r = new Registrant(h, what, obj);
        mSuspendedSetStatusRegistrants.add(r);
    }

    public void unregisterForSuspendedSetStatus(Handler h) {
        mSuspendedSetStatusRegistrants.remove(h);
    }
    /// @}
