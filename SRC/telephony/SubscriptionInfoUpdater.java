...
public class SubscriptionInfoUpdater extends Handler {
    ...
    private static final boolean MTK_C2K_SUPPORT = CdmaFeatureOptionUtils.isMtkC2KSupport();
    ...
    private int mOldSvlteSlotId = SvlteModeController.getActualSvlteModeSlotId();
    ...
    private final BroadcastReceiver sReceiver = new  BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (!action.equals(TelephonyIntents.ACTION_SIM_STATE_CHANGED) &&
                ...
                !action.equals(TelephonyIntents.ACTION_SET_RADIO_TECHNOLOGY_START) &&
                !action.equals(TelephonyIntents.ACTION_SET_RADIO_TECHNOLOGY_DONE)) {
                // MTK-END
                return;
            }
                ...
                if (mOldSvlteSlotId != svlteSlotId) {
                    for (int i = 0; i < mCis.length; i++) {
                        ...
                        logd("svlte" + i + " nlte ci:" +
                                SvlteUtils.getSvltePhoneProxy(i).getNLtePhone().mCi);
                        logd("svlte" + i + " active ci:" +
                                ((PhoneBase) SvlteUtils.getSvltePhoneProxy(i).
                                    getActivePhone()).mCi);
                    }
                    for (int i = 0; i < mCis.length; i++) {
                        sIsUpdateAvailable[i] = 0;
