...
public class SIMRecords extends IccRecords {
    ...
    private static final int EVENT_GET_O2PERSO_DONE = 37;//by wangkai 20151030 add for task730341
    // MTK-START
    ...
    */
    ...
    String mEfEcc = "";
    // MTK-END
    //begin wangkai 20151030 add for task730341
    private int sstBit5 = -1;
    private int sstBit6 = -1;
    private int o2PersoFlag = -1;

    private int mBroadband = 0;
    private int mTesco = 0;
    private int mGiffGaff = 0;
    //end wangkai 20151030 add for task730341
    // ***** Constructor
    ...
    public void handleMessage(Message msg) {
		...
		//begin-zhibin.zhong modify for task:707139
                // MTK-START
               /* } else if (isCphsMailboxEnabled()) {
                // MTK-END
			...
			//end-zhibin.zhong modify for task:707139
                // MTK-END
                }
            ...
            case EVENT_GET_SST_DONE:
                ...
                //begin wangkai 20151030 add for task730341
                sstBit5 = 0x10 & data[0];// is bite5 ==0 ?
                sstBit6 = 0x20 & data[0]; // is bite6 == 0?
                log("sstBit5 = " + sstBit5 + ", sstBit6 = " + sstBit6);
                //end wangkai 20151030 add for task730341
                ...
                fetchGbaRecords();
                // MTK-END
                fetchMbdn();	////zhibin.zhong modify for task:707139
                break;
            //begin wangkai 20151030 add for task730341
            case EVENT_GET_O2PERSO_DONE:
                log("Got EVENT_GET_O2PERSO_DONE");
                isRecordLoadResponse = true;
                ...
                o2PersoFlag = 0x01 & data[0];
                mBroadband = 0x10 & data[0];
                mTesco = 0x01 & data[1];
                mGiffGaff = 0x04 & data[1];
                log("O2PERSO: " + IccUtils.bytesToHexString(data) + ", o2PersoFlag = " + o2PersoFlag);
                break;
            //end wangkai 20151030 add for task730341
            ...
            */
            case EVENT_QUERY_MENU_TITLE_DONE:
                ...
                if (ar != null && ar.exception == null) {
                    ...
                    if (data != null && data.length >= 2) {
                        ...
                        /*-begin-20151020-yangning.hong-add-for-task730254 & 730263*/
                        int def = mContext.getResources().getInteger(com.android.internal.R.integer.def_customized_stk_app_name);
                        if (def == 2 && isOperatorSIM(mSlotId)) {//20160423-yangning.hong-add-for-D1924078-orange-stk-req
                            SystemProperties.set("gsm.stk.perso.app_title", mMenuTitleFromEf);
                        }
                        /*-end-20151020-yangning.hong-add-for-task730254 & 730263*/
                    }
                } else {
                    if (ar.exception != null) {
            ...
            case EVENT_RADIO_AVAILABLE:
                ...
                if (null != mTelephonyExt) {
                    if (mTelephonyExt.isSetLanguageBySIM()) {
                        fetchLanguageIndicator();
                    }
                } else {
                    loge("EVENT_RADIO_AVAILABLE: mTelephonyExt is null!!!");
                }
                mMsisdn = "";
                //setNumberToSimInfo();
                mRecordsEventsRegistrants.notifyResult(EVENT_MSISDN);
    ...
    private void handleSimRefresh(IccRefreshResponse refreshResponse){
        ...
        switch (refreshResponse.refreshResult) {
            ...
            case IccRefreshResponse.REFRESH_RESULT_INIT:
                ...
                /*begin-20151126-yangning.hong-remove-for-D958253--no-need-to-remove-Contacts-list-and-DB*/
                // MTK-START
                //setPhbReady(false);
                // MTK-END
                //onIccRefreshInit();
                /*end-20151126-yangning.hong-remove-for-D958253--no-need-to-remove-Contacts-list-and-DB*/
                break;
    ...
    //add by shilei.zhang for set system languaeg by sim before PIN code and fixed D1658045 begin
    private void setLocaleFromUsim() {
        String PrefLang = getSimLanguage();
        if (PrefLang != null) {
            String imsi = getIMSI();
            String country = null;
            if (imsi != null) {
                country = MccTable.countryCodeForMcc(Integer.parseInt(imsi.substring(0,3)));
                Log.d(LOG_TAG, "get prefLang is "+ PrefLang +"get IMSI, and country is " + country);
            }
            if (DBG) log("Setting locale to " + PrefLang + "_" + country);
            Log.d(LOG_TAG, "get prefLang is "+ PrefLang);
            MccTable.setSystemLocale(mContext, PrefLang, country);
        } else {
            if (DBG) log("No suitable USIM selection locale");
            Log.d(LOG_TAG, "No suitable USIM selection locale ");
        }
    }
    //add by shilei.zhang for set system languaeg by sim before PIN code and fixed D1658045 end
    @Override
    protected void onAllRecordsLoaded() {
        if (DBG) log("record load complete");
        o2PrepayOrPostpay();//by wangkai 20151030 add for task730341
        //add by shilei.zhang for set system languaeg by sim before PIN code and fixed D1658045 begin
        /*Resources resource = Resources.getSystem();
        ...
        /*-begin-20151020-yangning.hong-add-for-task730254 & 730263 & 730245*/
        if(operator != null ){
            Log.d(LOG_TAG, "SIMRecords: update stk name. operator = " + operator);
        ...
        /*-end-20151020-yangning.hong-add-for-task730254 & 730263 & 730245*/
...
//begin-zhibin.zhong modify for task:707139
    private void fetchMbdn(){
     	if (DBG) log("fetchMbdn()");

	    boolean bMdnActive = false;
	
        if (mEfSST != null) {  
	        if (mParentApp.getType() == AppType.APPTYPE_USIM) {
	            if (mEfSST.length >= 6) {
	                bMdnActive = ((mEfSST[5] & 0x40) == 0x40);
	            }
	        } else {
		        if (mEfSST.length >= 14) {
		            bMdnActive = ((mEfSST[13] & 0x03) == 0x03);
		        }
	        }
         }
      ...
      /*  mFh.loadEFLinearFixed(EF_MBI, 1, obtainMessage(EVENT_GET_MBI_DONE));
       ...
       //end-zhibin.zhong modify for task:707139
        ...
        /*
                ...
                */
        /*-begin-20151020-yangning.hong-add-for-task730254 & 730263*/
        if (2 == mContext.getResources().getInteger(com.android.internal.R.integer.def_customized_stk_app_name)) {
        ...
        /*-end-20151020-yangning.hong-add-for-task730254 & 730263*/
        //begin wangkai 20151030 add for task730341
        mFh.loadEFTransparent(EF_O2PERSO, obtainMessage(EVENT_GET_O2PERSO_DONE));
        mRecordsToLoad++;
        //end wangkai 20151030 add for task730341
        fetchCPHSOns();
        // MTK-END
               ...
               wipeAllSIMContacts();
                ...
                if (action.equals("action_pin_dismiss")) {
                   ...
                   if (null != mTelephonyExt) {
                       ...
                       loge("ACTION_SHUTDOWN_IPO: mTelephonyExt is null!!!");
                   }
        ...
        for (int i = 0 ; i < localsSize; i++) {
            ...
            if (len == 5) {
                ...
                if (lang.equals(language)) {
                    ret = true;
                    //be open by shilei.zhang for support sim language before Pincode confim
    ...
    */
    ...
    private void fetchRatBalancing() {
        ...
        if (null != mTelephonyExt) {
            ...
            loge("fetchRatBalancing: mTelephonyExt is null!!!");
        }
    ...
    public void broadcastPhbStateChangedIntent(boolean isReady) {
        ...
        if (subId <= 0) {
            ...
            if (DBG) log("Broadcasting intent ACTION_PHB_STATE_CHANGED " + isReady
                        + " subId " + subId + " phoneId " + mParentApp.getPhoneId());
    ...
    /*begin-20160423-yangning.hong-add-for-D1924078-orange-stk-req*/
    private boolean isOperatorSIM(int slotId){
        String operatorList = mContext.getResources().getString(com.android.internal.R.string.def_show_stk_menu_for_operators_list);
        if(TextUtils.isEmpty(operatorList)){
            log("def_show_stk_menu_for_operators_list is empty");
            return true;
        }
        String[] slotPlmns = SystemProperties.get("gsm.sim.operator.numeric", "").split(",");
        if(slotPlmns != null && slotPlmns.length > slotId && operatorList.contains(slotPlmns[slotId])){
           log("isOperatorSIM true, slot: " + slotId);
            return true;
        }
        log("isOperatorSIM false, slot: " + slotId);
        return false;
    }
    /*end-20160423-yangning.hong-add-for-D1924078-orange-stk-req*/

    // MTK-END
    //begin wangkai 20151030 add for task730341
    /*
     * Just for O2 prepay or postpay.
     * Read EF '6F38',and Byte 1
     * Fixed Dialling Numbers: Bit 5 = 1 (Allocated)
     * Bit 6 = 0 (Not Activated): PrePay
     * Bit 6 != 0 (Activated): PostPay
     * */
     private void o2PrepayOrPostpay() {
         String isPrepayOrPostpay = "";
         String PREPAY_OR_POSTPAY = "gsm.sim.operator.paykind" + mSlotId;
         if (o2PersoFlag == -1) {
             log("O2 Perso unhandled yet");
             return;
         }
         if (o2PersoFlag > 0) {
             log("Postpay From O2 Perso");
             isPrepayOrPostpay = "postpay";
         } else if (o2PersoFlag == 0) {
             log("Prepay From O2 Perso");
             if (mTesco == 0x01) {
                 isPrepayOrPostpay = "tesco";
             } else if (mGiffGaff == 0x04) {
                 isPrepayOrPostpay = "giffgaff";
             } else if (mBroadband == 0x10) {
                 isPrepayOrPostpay = "broadband";
             } else {
                 isPrepayOrPostpay = "prepay";
             }
         } else if (o2PersoFlag == -2) {
             if (sstBit5 == -1) {
                 log("No O2 Perso, SST unhandled yet");
                 return;
             }
             if (0 != sstBit5) {
                 if (0 == sstBit6) {
                     ...
                     log("Postpay From SST");
                     isPrepayOrPostpay = "postpay";
                 }
             }
         }
         log("isPrepayOrPostpay: " + isPrepayOrPostpay);
         SystemProperties.set(PREPAY_OR_POSTPAY, isPrepayOrPostpay);
     }
     //end wangkai 20151030 add for task730341
}
