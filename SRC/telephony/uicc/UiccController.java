...
public class UiccController extends Handler {
    ...
    private UiccController(Context c, CommandsInterface []ci) {
        ...
        mContext = c;
        mCis = new CommandsInterface[ci.length];
    ...
    /*
     * wait eusim ready to notify cardtype
     */
    ...
    private void initC2KWPCardtype(Integer index) {
        ...
        if (phoneCount == 1) {
            ...
            if (index == 0) {
                ...
                cardTypeReady = cardTypeSet2.equals("1");
            } else {
                cardTypeReady = cardTypeSet.equals("1") && cardTypeSet2.equals("1");
            }
        }
        if (cardTypeReady) {
            updateC2KWPCardtype(true);
            SystemProperties.set("gsm.ril.init", "0");
            log("initC2KWPCardtype set gsm.ril.init to 0");
        } else {
        ...
        synchronized (mLock) {
            ...
            switch (msg.what) {
                case EVENT_ICC_STATUS_CHANGED:
                    ...
                    if (CdmaFeatureOptionUtils.isCdmaLteDcSupport()) {
                        ...
                        if (mSetRadioDone == false) {
                            log("EVENT_ICC_STATUS_CHANGED, index: " + index
                                + " radio not done, sendDelay");
                            sendMessageDelayed(obtainMessage(EVENT_ICC_STATUS_CHANGED, index),
                                1000);
                            break;
                        } else if ((SvlteModeController.getActualSvlteModeSlotId()
                            != SvlteModeController.CSFB_ON_SLOT)
                            && (index == mSvlteIndex) && (mC2KWPCardtype[index] != 0)
                            && (!configSend.equals("1") || iccidC2k.equals(""))) {
                            log("EVENT_ICC_STATUS_CHANGED, index: " + index
                                + " c2k not ready, sendDelay");
                ...
                case EVENT_VIRTUAL_SIM_ON:
                    ...
                    //begin gchenli add for task730491 on 20151119
                    ...
                    android.provider.Settings.Global.putInt(mContext.getContentResolver(),
                            android.provider.Settings.Global.CALL_AUTO_ANSWER,
                            1);
                    //end gchenli add for task730491 on 20151119
                    break;
                        ...
                        if ((phoneCount > 1 && allAreCdmaSim &&
                            ...
                            */
                            for (int i = 0; i < mC2KWPCardtype.length; i++) {
                                ...
                                Rlog.d(LOG_TAG, "EVENT_SIM_PLUG_OUT mC2KWPCardtype[" + i + "]="
                                    + mC2KWPCardtype[i]);
                            }
                        } else {
                            updateC2KWPCardtype(true);
                ...
                case EVENT_EUSIM_READY:
                    ...
                    if (CdmaFeatureOptionUtils.isCdmaLteDcSupport()) {
                        ...
                        if ((phoneCount >= 2) && (!mSwitchCardtype) && rilInit.equals("1")
                            ...
                            SystemProperties.set("gsm.ril.init", "0");
                            mRilInit = true;
                            log("set gsm.ril.init to 0, mRilInit = " + mRilInit);
                            break;
                        } else {
                            SystemProperties.set("gsm.ril.init", "0");
                            mRilInit = false;
                            log("Clear gsm.ril.init to 0, mRilInit = " + mRilInit);
    ...
    private final BroadcastReceiver mIntentReceiver = new BroadcastReceiver() {
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(ACTION_RESET_MODEM)) {
                ...
                int svlteSlotId = SvlteModeController.getActualSvlteModeSlotId();
                mSetRadioDone = false;
                log("Receive ACTION_SET_RADIO_TECHNOLOGY_START mOldSvlteSlotId: "
                    + mOldSvlteSlotId + "  svlteSlotId: " + svlteSlotId
                    + " mSetRadioDone: " + mSetRadioDone);
                ...
                if (svlteSlotId == SvlteModeController.CSFB_ON_SLOT) {
                    if (mOldSvlteSlotId != SvlteModeController.CSFB_ON_SLOT) {
                    ...
                    //Set svlteCi
                    if (mSvlteIndex != -1) {
                        setSvlteCi(SvlteUtils.getSvltePhoneProxy(mSvlteIndex).getLtePhone().mCi);
                    }
                    mOldSvlteSlotId = svlteSlotId;
                } else if (mOldSvlteSlotId != svlteSlotId) {
                    if (mOldSvlteSlotId != SvlteModeController.CSFB_ON_SLOT) {
                        unSetSvlteCi(mSvlteCi);
                    }
                    //Unset mCi0 and mCi1
                    ...
                    if (mCis.length == 1) {
                        ...
                        mCis[svlteSlotId] = SvlteUtils.getSvltePhoneProxy(svlteSlotId)
                            .getNLtePhone().mCi;
                        if (svlteSlotId == 0) {
                ...
                if (svlteSlotId == SvlteModeController.CSFB_ON_SLOT) {
                    ...
                    if (mCis.length == 1) {
                        ...
                        tempCis[svlteSlotId] = SvlteUtils.getSvltePhoneProxy(svlteSlotId)
                            .getNLtePhone().mCi;
                        if (svlteSlotId == 0) {
                ...
                for (int i = 0; i < tempCis.length; i++) {
                    log("tempCis[" + i + "]: " + ((null != tempCis[i]) ?
                        tempCis[i].hashCode() : null));
                }
                if (mCis.length == 1) {
                    if ((mCis[0] != null && !mCis[0].equals(tempCis[0])) ||
                        (tempCis[0] != null && !tempCis[0].equals(mCis[0]))) {
                        log("at least one CI changed");
                        log("slot[0].activephone = " + SvlteUtils.getSvltePhoneProxy(0)
                            .getActivePhone());
                        log("mCis[0] = " + mCis[0]);
                        log("tempCis[0] = " + tempCis[0]);
                        if ((tempCis[0] == null)) {
                            log("at least one change as null, unSetCis/setCis limitation, abort");
                            return;
                        }
                    ...
                    if ((mCis[0] != null && !mCis[0].equals(tempCis[0])) ||
                        ...
                        log("slot[0].activephone = " + SvlteUtils.getSvltePhoneProxy(0)
                            .getActivePhone());
                        log("slot[1].activephone = " + SvlteUtils.getSvltePhoneProxy(1)
                            .getActivePhone());
                        log("mCis[0] = " + mCis[0]);
                        log("tempCis[0] = " + tempCis[0]);
                        log("mCis[1] = " + mCis[1]);
    ...
    public void setCis(CommandsInterface []ci, boolean init) {
        ...
        for (int i = 0; i < mCis.length; i++) {
            ...
            if (CdmaFeatureOptionUtils.isCdmaLteDcSupport()
                && (i == SvlteModeController.getActualSvlteModeSlotId())) {
    ...
    /*
     * wait eusim ready to register then to skip the eusim notify
     * during power on
     */
    ...
    @Deprecated public IccCardConstants.CardType getCardType() {
        ...
        if (mCdmaCardType == IccCardConstants.CardType.UNKNOW_CARD) {
            String cardTypeCache = SystemProperties.get("ril.cardtype.cache", "");
            Rlog.d(LOG_TAG, "getCardType cardTypeCache:" + cardTypeCache);
            if (!cardTypeCache.equals("")) {
                try {
                    int cardtype = Integer.parseInt(cardTypeCache);
                    mCdmaCardType = IccCardConstants.CardType.getCardTypeFromInt(cardtype);
                } catch (NumberFormatException e) {
                    Rlog.e(LOG_TAG, "getCardType Fail to parse cardtype");
                }
            }
        }
    ...
    private void updateC2KWPCardtype(boolean force) {
        ...
        for (int i = 0; i < mC2KWPCardtype.length; i++) {
            ...
            if (CdmaFeatureOptionUtils.isCdmaLteDcSupport()
                ...
                if (sProxyPhones != null) {
                    mIccCardProxy = (IccCardProxy) (((SvltePhoneProxy) sProxyPhones[i])
                        .getIccCard());
        ...
        Rlog.d(LOG_TAG, "updateC2KWPCardtype force=" + force + ", gsm.ril.cardtypeset="
            + cardTypeSet + ", gsm.ril.cardtypeset.2=" + cardTypeSet2
            + ", ril.cdma.switch.needupdate=" + switchNeedUpdate + ", mSetTrm=" + mSetTrm);

        if (force || cardTypeReady) {
            //Clear TRM flag
            if (mSetTrm) {
                mSetTrm = false;
                Rlog.d(LOG_TAG, "updateC2KWPCardtype mSetTrm=" + mSetTrm);
            }
    ...
    private void setTrm() {
        mSetRadioDone = false;
        mSetTrm = true;
        log("setTrm, mSetRadioDone=" + mSetRadioDone + " mSetTrm=" + mSetTrm);
        //Clear property
