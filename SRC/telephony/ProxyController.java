...
public class ProxyController {
    ...
    public boolean setRadioCapability(RadioAccessFamily[] rafs) {
        ...
        if (rafs.length != mProxyPhones.length) {
            logd("Length of input rafs must equal to total phone count");
        ...
        if (bIsboth3G == false) {
            logd("input parameter is incorrect - no 3g phone");
        ...
        if (CdmaFeatureOptionUtils.isCdmaLteDcSupport()) {
            ...
            logd("setRadioCapability,and isSvlteModeSwitching = "+isSvlteModeSwitching);
            if (isSvlteModeSwitching){
                //ModeController switching now,wait for next trigger
                logd("setRadioCapability,ModeSwitching wait for next trigger");
                return true;
            }
            // check if sim locked
            int simState = 0;
            ...
            for (int i = 0; i < simNum; i++) {
                ...
                if (simState == TelephonyManager.SIM_STATE_PIN_REQUIRED ||
                    ...
                    logd("setRadioCapability,sim is locked simState = " + simState);
                }
            }
            if(skipCapabilitySwitch){
                logd("setRadioCapability,sim is locked wait for unlock trigger");
                return true;
            }
        ...
        if (CdmaFeatureOptionUtils.isCdmaLteDcSupport()) {
            ...
            //Set sim switching flag for ModeController check,
            //will be reset when ril init after TRM
            SystemProperties.set("ril.cdma.switching", "1");
        }
