...
/// M: For RIL_UNSOL_IMEI_LOCK notification
...
public final class RIL extends BaseCommands implements CommandsInterface {
    ...
    private final DisplayManager.DisplayListener mDisplayListener =
        ...
        public void onDisplayChanged(int displayId) {
            if (RILJ_LOGD) {
                riljLog("onDisplayChanged: displayId = "
                        + displayId);
            }
        ...
        if (CdmaFeatureOptionUtils.isCdmaLteDcSupport()) {
            ...
            if (cm.isNetworkSupported(ConnectivityManager.TYPE_MOBILE) == false) {
                ...
                new Thread() {
                    public void run() {
                        try {
                            riljLog("before parse plmn-list.config");
                            SpnOverride.getInstance().getPlmnFromFile();
                            riljLog("after parse plmn-list.config");
                        } catch (Exception e) {
                            riljLog(e.toString());
                        }
                   }
    ...
    private void updateScreenState() {
        ...
        mRadioScreenState = (mDefaultDisplayState != Display.STATE_OFF)
 ...
 | sed -re 's/\{([^,]+),[^,]+,([^}]+).+/case \1: ret = \2(p); break;/'
            ...
            case RIL_REQUEST_SET_CLIP: ret = responseVoid(p); break;
...
*/
            ...
            case RIL_UNSOL_CDMA_CARD_INITIAL_ESN_OR_MEID: ret = responseString(p); break;
            // MTK-END, SMS part
        ...
        switch(response) {
            ...
            case RIL_UNSOL_RESPONSE_NEW_BROADCAST_SMS:
                ...
                //add by weiwen.huang.hz 20151230 1177030 begin
                else{
                    riljLogv("weiwen mGsmBroadcastSmsRegistrant is null");
                }
                //add by weiwen.huang.hz 20151230 1177030 end
                break;
            ...
            case RIL_UNSOL_CDMA_CARD_INITIAL_ESN_OR_MEID:
                if (RILJ_LOGD) unsljLog(response);
                if (mCDMACardEsnMeidRegistrant != null) {
                    mCDMACardEsnMeidRegistrant.notifyRegistrant(new AsyncResult(null, ret, null));
                } else {
                    mEspOrMeid = ret;
                }
                break;
    ...
    responseCallList(Parcel p) {
        ...
        for (int i = 0 ; i < num ; i++) {
          ...
          //begin wwanying modify for 730344 on 20151125
            riljLogv("dc.state:" + dc.state + ", dc.TOA:" + dc.TOA
                 + ", dc.isVoice:" + dc.isVoice + ", dc.isVoicePrivacy:" + dc.isVoicePrivacy
                 + ", dc.number" + dc.number + ", dc.numberPresentation:" + dc.numberPresentation
                 + ", dc.name:" + dc.name + ", dc.namePresentation:" + dc.namePresentation
                 + ", uusInfoPresent:" + uusInfoPresent);
            //end wwanying modify for 730344 on 20151125
    ...
    responseOperatorInfosWithAct(Parcel p) {
        ...
        for (int i = 0 ; i < strings.length ; i += 5) {
            ...
            if(mContext.getResources().getBoolean(com.android.internal.R.bool.def_need_display_short_name_when_long_and_short_name_exist)) {
                if (shortName != null) {
                    ...
                    if (lacValue != 0xfffe) {
                        ...
                        if (sEons != null && !sEons.equals("")) {
                            ...
                            if ((mSimOperatorNumeric != null) &&
                                ...
                                //begin wangkai 20151113 modify for merging defect233812
                                //old: if (sCphsOns != null) {
                                if (!TextUtils.isEmpty(sCphsOns)) {
                                //end wangkai 20151113 modify for merging defect233812
            ...
            // ALPS00353868 END
          //begin wangkai 20151024 add for task730514
            boolean disable2g3gDistinguish = mContext.getResources().getBoolean(com.android.internal.R.bool.def_disable_2G_3G_distinguish);
            if (disable2g3gDistinguish) {
                if (i > 0 && strings[i + 0] != null) {
                    boolean isDiscarded = false;
                    for (int j = 0; j < i; j += 5) {
                        if (strings[j + 0] != null && strings[i + 0].equals(strings[j + 0])) {
                            isDiscarded = true;
                            break;
                        }
                    }
            ...
            //end wangkai 20151024 add for task730514
            ...
            if (("334050".equals(strings[i + 2]))
                    && ("2G".equals(strings[i + 4]))
                    && (strings[i + 0] != null && strings[i + 0].contains("Iusacell"))) {//by wangkai 20151111 add for task730589
                strings[i + 0] = strings[i + 1] = "Iusacell GSM";
            }
            if (("334050".equals(strings[i + 2]))
                    && ("3G".equals(strings[i + 4]))
                    && (strings[i + 0] != null && strings[i + 0].contains("Iusacell"))) {
                    strings[i + 0] = strings[i + 1] = "Iusacell 4G 3G"; //by wangkai 20151112 add for customising 334050 Iusacell names
            }
            if (("74810".equals(strings[i + 2]))
                    && ("4G".equals(strings[i + 4]))) {
                strings[i + 0] = strings[i + 1] = "Claro LTE";
            }
...
*/
    ...
    */
    ...
    public String lookupOperatorNameFromNetwork(long subId, String numeric, boolean desireLongName) {
        ...
        nitzOperatorNumeric = TelephonyManager.getTelephonyProperty(phoneId, TelephonyProperties.PROPERTY_NITZ_OPER_CODE, null);//by wangkai 20151024 modify as null from "" for task730426
        if ((numeric != null) && (numeric.equals(nitzOperatorNumeric))) {
            if (desireLongName == true) {
                nitzOperatorName = TelephonyManager.getTelephonyProperty(phoneId, TelephonyProperties.PROPERTY_NITZ_OPER_LNAME, null);//by wangkai 20151024 modify as null from "" for task730426
            } else {
                nitzOperatorName = TelephonyManager.getTelephonyProperty(phoneId, TelephonyProperties.PROPERTY_NITZ_OPER_SNAME, null);//by wangkai 20151024 modify as null from "" for task730426
            }
        }
