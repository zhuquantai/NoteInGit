...
public class CdmaSMSDispatcher extends SMSDispatcher {
    ...
    private static final int EVENT_QUERY_CDMA_NETWORK_REGISTER_STATE =
                                                EVENT_CDMA_SMS_DISPATCHER_BASE + 3;
    private static final int EVENT_QUERY_CDMA_NETWORK_REGISTER_STATE_DONE =
                                                EVENT_CDMA_SMS_DISPATCHER_BASE + 4;
    private static final int EVENT_QUERY_CDMA_MODEM_SMS_INIT_STATE =
                                                EVENT_CDMA_SMS_DISPATCHER_BASE + 5;
    private static final int EVENT_QUERY_CDMA_MODEM_SMS_INIT_STATE_DONE =
                                                EVENT_CDMA_SMS_DISPATCHER_BASE + 6;

    // MTK-START
    private static final int EVENT_CDMA_CARD_INITIAL_ESN_OR_MEID =
                                                EVENT_CDMA_SMS_DISPATCHER_BASE + 7;
    // MTK-END
    ...
    public CdmaSMSDispatcher(PhoneBase phone, SmsUsageMonitor usageMonitor,
        ...
        if (mUiccController != null) {
            if (phone.getPhoneType() == PhoneConstants.PHONE_TYPE_CDMA) {
                ...
                Rlog.d(TAG, "viacode, mUiccController != null," +
                        " register for icc change successly for CDMA");
            } else {
        ...
        // VIA add end
        // MTK-START
        mCi.setCDMACardInitalEsnMeid(this, EVENT_CDMA_CARD_INITIAL_ESN_OR_MEID, null);
        // MTK-END
    ...
    private void notifyCdmaSmsAutoRegisterBeFeasible() {
        if (!mCdmaNetworkRegistered || !mCdmaModemSmsInitDone) {
               ...
               Rlog.e(TAG, "viacode, mCdmaNetworkRegistered = " + mCdmaNetworkRegistered +
                       " , mCdmaModemSmsInitDone = " + mCdmaModemSmsInitDone);
               return;
        }
        ...
        //temp add, common will add later
        PowerManager.WakeLock wakeLock;
        PowerManager pm = (PowerManager) mContext.getSystemService(Context.POWER_SERVICE);
        wakeLock = pm.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "CDMASMSDispatcher");
    ...
    private void onUpdateIccAvailability() {
        ...
        int slotId = CdmaFeatureOptionUtils.getExternalModemSlot();
        Rlog.d(TAG, "CdmaSMSDispatcher received EVENT_ICC_CHANGED, slotId = " + slotId);
        UiccCardApplication newUiccApplication =
                mUiccController.getUiccCardApplication(slotId, UiccController.APP_FAM_3GPP2);
    ...
    public void handleMessage(Message msg) {
        ...
        case EVENT_RUIM_READY:
            if (!mSmsRegisterFeasibilityQuerying) {
                ...
                mCi.queryCDMANetWorkRegistrationState(obtainMessage(
                        EVENT_QUERY_CDMA_NETWORK_REGISTER_STATE_DONE));
            } else {
                Rlog.e(TAG, "viacode, received EVENT_RUIM_READY," +
                    " but query processing, dont need query multi.");
            }
            break;
        case EVENT_QUERY_CDMA_NETWORK_REGISTER_STATE:
            mQueryCDMASmsRegisterFeasibilityTimes++;
            Rlog.d(TAG, "viacode, do network register state query again, retry times = " +
                    mQueryCDMASmsRegisterFeasibilityTimes);
            if (mQueryCDMASmsRegisterFeasibilityTimes >
                    QUERY_SMS_REGISTER_FEASIBILITY_MAX_RETRY_TIMES) {
            ...
            if (mSmsRegisterFeasibilityQuerying) {
                ...
                if (aresult != null) {
                    ...
                    if (res != null && res.length == 1 && res[0] == 1) {
                        ...
                        sendEmptyMessageDelayed(EVENT_QUERY_CDMA_NETWORK_REGISTER_STATE,
                                QUERY_SMS_REGISTER_FEASIBILITY_DELAY_MILLISECONDS);
                    }
                } else {
                    // not ok , query again after a moment
                    sendEmptyMessageDelayed(EVENT_QUERY_CDMA_NETWORK_REGISTER_STATE,
                ...
                Rlog.e(TAG, "viacode, received EVENT_QUERY_CDMA_NETWORK_REGISTER_STATE_DONE" +
                        " but is not querying");
        ...
        case EVENT_QUERY_CDMA_MODEM_SMS_INIT_STATE:
            mQueryCDMASmsRegisterFeasibilityTimes++;
            Rlog.d(TAG, "viacode, do sms init state query again, retry times = " +
                    mQueryCDMASmsRegisterFeasibilityTimes);
            if (mQueryCDMASmsRegisterFeasibilityTimes >
                    QUERY_SMS_REGISTER_FEASIBILITY_MAX_RETRY_TIMES) {
                Rlog.e(TAG, "over retry limits(" + QUERY_SMS_REGISTER_FEASIBILITY_MAX_RETRY_TIMES +
                ...
                mCi.queryCDMASmsAndPBStatus(obtainMessage(
                        EVENT_QUERY_CDMA_MODEM_SMS_INIT_STATE_DONE));
            }
            break;
            ...
            if (mSmsRegisterFeasibilityQuerying) {
                ...
                if (asyncresult != null) {
                    ...
                    if (queryres != null && queryres.length == 2 && queryres[0] == 1) {
                        ...
                        // not ok , query again after a moment
                        sendEmptyMessageDelayed(EVENT_QUERY_CDMA_MODEM_SMS_INIT_STATE,
                                QUERY_SMS_REGISTER_FEASIBILITY_DELAY_MILLISECONDS);
                    }
                } else {
                    // not ok , query again after a moment
                ...
                Rlog.e(TAG, "viacode, received EVENT_QUERY_CDMA_MODEM_SMS_INIT_STATE_DONE" +
                        " but is not querying");
                Rlog.e(TAG, "viacode, must make sure is retrying in the same instance," +
                        " drop this done EVENT if is not retry querying.");
            }
    ...
    /** {@inheritDoc} */
    @Override
    protected TextEncodingDetails JRDcalculateLength(CharSequence msgBody,
            boolean use7bitOnly, int encodeMode) {
        Rlog.i(TAG,"JRDcalculateLength -> CdmaSMSDispatcher JRDcalculateLength");
            //TODO
        return null;
    }
    ...
    protected SmsTracker getNewSubmitPduTracker(String destinationAddress, String scAddress,
        ...
        } else if (encoding == SmsConstants.ENCODING_8BIT) {
            uData.msgEncoding = UserData.ENCODING_OCTET;
        } else { // assume UTF-16
            uData.msgEncoding = UserData.ENCODING_UNICODE_16;
        }
        uData.msgEncodingSet = true;
        Rlog.d(TAG, "viacode: to use the encoding type [" + uData.msgEncoding + "]");
        ...
        if (submitPdu == null) {
            Rlog.d(TAG, "CDMASMSDispatcher.getNewSubmitPduTracker(), L," +
                    " getSubmitPdu() returned null, A");
            return null;
        } else {
    ...
    protected SmsTracker getNewSubmitPduTracker(String destinationAddress, String scAddress,
        ...
        if (submitPdu != null) {
            ...
            Rlog.e(TAG, "CDMASMSDispatcher.getNewSubmitPduTracker(), L," +
                    " getSubmitPdu() returned null, B");
            return null;
        }
    }
    // MTK-END
    ...
    protected void sendSmsByPstn(SmsTracker tracker) {
        ...
        if (!isIms() && ss != ServiceState.STATE_IN_SERVICE) {
            // MTK-START
            // If user plugs out the card when send the long sms, we need
            // an chance to notify the caller and clear the track queue.
            synchronized (mSTrackersQueue) {
                while (!mSTrackersQueue.isEmpty()) {
                    Rlog.d(TAG, "updatePhoneObject, remove tracker.");
                    mSTrackersQueue.remove(0).onFailed(mContext,
                            RESULT_ERROR_GENERIC_FAILURE, 0/*errorCode*/);
                }
            }
            //MTK-END
            return;
    ...
    protected void sendData(String destAddr, String scAddr, int destPort, int originalPort,
            byte[] data, PendingIntent sentIntent, PendingIntent deliveryIntent) {
        // impl
        Rlog.d(TAG, "viacode: CdmaSMSDispatcher, implemented by VIA for interfaces needed." +
                " sendData");
        SmsMessage.SubmitPdu pdu = SmsMessage.getSubmitPdu(
    ...
    protected void sendMultipartTextWithEncodingType(String destAddr, String scAddr,
        ...
        for (int i = 0; i < msgCount; i++) {
            ...
            Rlog.d(TAG, "viacode: to send the " + i + " part");
            trackers[i] =
    ...
    public void sendTextWithExtraParams(String destAddr, String scAddr, String text,
        ...
        Rlog.d(TAG, "viacode: priority is " + priority);
        Rlog.d(TAG, "viacode: want to use encoding = " + encoding);
    ...
    protected android.telephony.SmsMessage createMessageFromSubmitPdu(byte[] smsc, byte[] tpdu) {
        ISmsInterfaces mSmsInterfaces = ViaPolicyManager.getSmsInterfaces();
        if (mSmsInterfaces == null) {
            Rlog.d(TAG, "createMessageFromSubmitPdu, mSmsInterfaces = null");
            return null;
        }
        return android.telephony.SmsMessage.createFromPdu(
                mSmsInterfaces.convertSubmitpduToPdu(tpdu), getFormat());
    }
}
