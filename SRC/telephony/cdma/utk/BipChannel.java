...
class TcpClientChannel extends BipChannel {
    ...
    TcpClientChannel(BipService bs, OpenChannelSettings p, int id, Context context) {
        super(bs, p, id);
        mContext = context;
    }
    ...
    public int linkEstablish() {
        ...
        try {
            ...
            if (network != null) {
                mSocket = network.getSocketFactory().createSocket();
            } else {
                UtkLog.d("network is null", " exception");
                mSocket = new Socket();
            }
...
class TcpServerChannel extends TcpClientChannel {
    ...
    TcpServerChannel(BipService bs, OpenChannelSettings p, int id, Context context) {
        super(bs, p, id, context);
...
class UdpClientChannel extends BipChannel {
    ...
    UdpClientChannel(BipService bs, OpenChannelSettings p, int id, Context context) {
        //
        super(bs, p, id);
    ...
    public int linkEstablish() {
        ...
        try {
            ...
            if (network != null) {
                ...
                UtkLog.d("UdpClientChannel", " network is null");
            ...
            UtkLog.d("UdpClientChannel", " IOException");
            return BipConstants.RESULT_ERROR;
        }
...
abstract class BipRunnable implements Runnable {
    ...
    public boolean compAndCache(byte[] data) {
        ...
        String cb = IccUtils.bytesToHexString(mCacheBuf);
        String cd = IccUtils.bytesToHexString(data);

        boolean isSame = false;
        try {
            if (cb == null) {
                UtkLog.d("BipRunnable", " compAndCache: mCacheBuf = null, maybe first");
            } else {
                isSame = cb.equals(cd);
            }
            if (isSame) {
                UtkLog.d("BipRunnable", " compAndCache: mCacheBuf = " + cb);
                UtkLog.d("BipRunnable", " compAndCache: data = " + cd);
            }
            if (data != null && data.length > 0) {
                mCacheBuf = new byte[data.length];
                Arrays.fill(mCacheBuf, (byte) 0);
                System.arraycopy(data, 0, mCacheBuf, 0, data.length);
            } else {
                UtkLog.d("BipRunnable", " compAndCache: data = null");
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
