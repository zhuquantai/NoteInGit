...
abstract class ValueParser {
    ...
    /////
    static byte[] retrieveSetupEventList(ComprehensionTlv ctlv) throws ResultException {
        byte[] eventList = null;
        ...
        try {
            ...
            if (valueLen > 0) {
                ...
                System.arraycopy(rawValue, valueIndex, eventList, 0, valueLen);
            }
        } catch (IndexOutOfBoundsException e) {
            throw new ResultException(ResultCode.CMD_DATA_NOT_UNDERSTOOD);
        }
    ...
    static BearerDescription retrieveBearerDesc(ComprehensionTlv ctlv) throws ResultException {
        BearerDescription bearerDesc = null;
        int bearerType;
        byte[] bearerParams = null;
        ...
        try {
            ...
            bearerType = rawValue[valueIndex++] & 0xff;

            if (valueLen > 1) {
                bearerParams = new byte[valueLen - 1];
                System.arraycopy(rawValue, valueIndex, bearerParams, 0, valueLen - 1);
            }

            bearerDesc = new BearerDescription(valueLen, bearerType, bearerParams);
    ...
    static int retrieveBufSize(ComprehensionTlv ctlv) throws ResultException {
        byte[] rawValue = ctlv.getRawValue();
        int valueIndex = ctlv.getValueIndex();
        int size = 0;
    ...
    static TransportLevel retrieveTransportLevel(ComprehensionTlv ctlv)
            throws ResultException {
        byte[] rawValue = ctlv.getRawValue();
        int valueIndex = ctlv.getValueIndex();
        TransportLevel tl = new TransportLevel();

        try {
            tl.protocolType = rawValue[valueIndex++];
            tl.port = ((rawValue[valueIndex] & 0xff) << 8) + (rawValue[valueIndex + 1] & 0xff);
        } catch (IndexOutOfBoundsException e) {
            throw new ResultException(ResultCode.CMD_DATA_NOT_UNDERSTOOD);
        }
    ...
    static int retrieveReqDataLength(ComprehensionTlv ctlv) throws ResultException {
        byte[] rawValue = ctlv.getRawValue();
        int valueIndex = ctlv.getValueIndex();
        int length = 0;
    ...
    static byte[] retrieveDataToSend(ComprehensionTlv ctlv) throws ResultException {
        byte[] rawValue = ctlv.getRawValue();
        int valueIndex = ctlv.getValueIndex();
        int dataLen = ctlv.getLength();
    ...
    3 to 3+X-1 Network Access Name X
      - is used to identify the Gateway entity, which provides interworking with an
        external packet data network.Defined by the different access technologies.
    */
    static String retrieveNAN(ComprehensionTlv ctlv) throws ResultException {
        byte[] rawValue = ctlv.getRawValue();
        int valueIndex = ctlv.getValueIndex();
        String networkAccessName = null;

        try {
            int len = ctlv.getLength();
            if (len > 0) {
                networkAccessName = new String(rawValue, valueIndex, len);
            }
        } catch (IndexOutOfBoundsException e) {
            throw new ResultException(ResultCode.CMD_DATA_NOT_UNDERSTOOD);
        }
    ...
    static OtherAddress retrieveOtherAddress(ComprehensionTlv ctlv) throws ResultException {
        ...
        try {
            ...
            if (BipConstants.BIP_OTHER_ADDRESS_TYPE_IPV4 == addressType) {
                try {
                    otherAddress = new OtherAddress();
                    otherAddress.addressType = addressType;
                    byte[] rawAddress = new byte[4];
                    System.arraycopy(rawValue, valueIndex, rawAddress, 0, rawAddress.length);

                    otherAddress.address = InetAddress.getByAddress(rawAddress);
                    } catch (UnknownHostException e) {

                    return null;
                }
            } else if (BipConstants.BIP_OTHER_ADDRESS_TYPE_IPV6 == addressType) {
                //
            } else {
                //
            }
