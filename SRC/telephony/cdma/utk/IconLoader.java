...
class IconLoader extends Handler {
    ...
    private IccFileHandler mRuimFH = null;
    ...
    private IconLoader(Looper looper , IccFileHandler fh, int phoneId) {
        super(looper);
        mRuimFH = fh;
        mPhoneId = phoneId;
    ...
    static IconLoader getInstance(Handler caller, IccFileHandler fh,  int phoneId) {
        ...
        if (fh != null) {
            if (null == sThread[phoneId]) {
                sThread[phoneId] = new HandlerThread("Utk Icon Loader");
                sThread[phoneId].start();
            }
            if (null != sThread[phoneId].getLooper()) {
                UtkLog.d("IconLoader", "Create IconLoader intance" + phoneId);
                return new IconLoader(sThread[phoneId].getLooper(), fh, phoneId);
            } else {
                return null;
            }
            //HandlerThread thread = new HandlerThread("Utk Icon Loader");
            //thread.start();
    ...
    public void handleMessage(Message msg) {
        ...
        try {
            ...
            UtkLog.d(this, "Icon load failed");
    ...
    private void readClut() {
        ...
        mRuimFH.loadEFImgTransparent(mId.imageId,
    ...
    public static Bitmap parseToBnW(byte[] data, int length) {
        ...
        if (pixelIndex != numOfPixels) {
            UtkLog.d("IconLoader", "parseToBnW; size error");
        }
