    ...
    private static int calculateNetworkType(Context context, int phoneId) {
        ...
        if (sActiveSvlteModeSlotId == phoneId) {
            ...
            if (phoneId == capabilityPhoneId) {
                ...
                /// M: SVLTE solution2 modify, avoid both slots are calculated to CDMA . @{
                if (TelephonyManager.getPhoneType(networkMode) == PhoneConstants.PHONE_TYPE_CDMA) {
                    networkMode = RILConstants.PREFERRED_NETWORK_MODE;
                }
                /// @}
            } else {
